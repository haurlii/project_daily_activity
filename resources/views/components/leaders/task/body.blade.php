    ...
    <table class="w-full text-theme-xs text-left text-gray-500 dark:text-gray-400">
        <thead class="text-theme-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-4 py-3 whitespace-nowrap">Batas Pengerjaan</th>
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                <th scope="row"
                    class="px-4 py-3 font-medium text-gray-900 max-w-lg truncate whitespace-nowrap dark:text-white/90">
                    {{ $loop->iteration }}
                </th>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3 mr-3 whitespace-nowrap max-w-xl">
                        ...
                        <div>
                            <span class="block font-medium text-gray-900 text-theme-xs dark:text-white/90">
                    ...
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-xl">
                        <div>
                            <span class="block font-medium text-gray-900 text-theme-sm dark:text-white/90">
                                {{ Str::of($task->title)->limit(30) ?? 'Tidak tersedia' }}
                            </span>
                            <span class="block">
                                {{ Str::of($task->description)->limit(60) ?? 'Tidak tersedia' }}
                            </span>
                        </div>
                    </div>
                </td>
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 max-w-lg">
                        {{ Str::of($task->start_date->translatedFormat('d M Y, H:i') . ' - ' .
                        $task->end_date->translatedFormat('d M Y, H:i')) }}
                    </div>
                </td>
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                        ...
                        {{ $task->activity->started_at->format('d F Y, H:i') }}
                        @break
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                        @switch ($task->status)
                        @case ( \App\Enums\StatusTask::SUCCESS )
                        {{ $task->activity->completed_at->format('d F Y, H:i') }}
                        @break
                        @case ( \App\Enums\StatusTask::LATE )
                        {{ 'Terlambat' }}
                        @break
                ...
                <td class="px-6 py-3 flex items-center justify-end">
                    ...
                    <div id="task-{{ $task->id }}-dropdown"
                        ...
                        <ul class="py-1 text-sm" aria-labelledby="task-{{ $task->id }}-dropdown-button">
                            {{-- <li>
                                <a href="{{ route('leader.tasks.show', $task->id) }}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600
            ...
            <div id="deletetaskModal-{{ $task->id }}"
                ...
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="deletetaskModal-{{ $task->id }}"
                        data-modal-toggle="deletetaskModal-{{ $task->id }}"
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                            Apakah Anda yakin ingin menghapus data tugas yang satu ini?
                        </p>

                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deletetaskModal-{{ $task->id }}"
                                data-modal-toggle="deletetaskModal-{{ $task->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="{{ route('leader.tasks.destroy', $task->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
