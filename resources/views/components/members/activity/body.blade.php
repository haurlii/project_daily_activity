    ...
    <table class="w-full text-theme-xs text-left text-gray-500 dark:text-gray-400">
        <thead class="text-theme-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-4 py-3 whitespace-nowrap">Aktivitas</th>
                <th scope="col" class="px-4 py-3 whitespace-nowrap">Waktu Pengerjaan</th>
                <th scope="col" class="px-4 py-3 whitespace-nowrap">Waktu PEngumpulan</th>
        ...
        <tbody>
            ...
            <tr class="border-b dark:border-gray-700">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-xl">
                        <div>
                            <span class="block font-medium text-gray-900 text-theme-sm dark:text-white/90">
                                {{ Str::of($activity->title)->limit(90) ?? 'Tidak tersedia' }}
                            </span>
                            <span class="block">
                                {{ Str::of($activity->description)->limit(120) ?? 'Tidak tersedia' }}
                            </span>
                        </div>
                    </div>
                </td>
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                        ...
                        {{ $activity->started_at->format('d F Y, H:i') }}
                        @break
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 whitespace-nowrap max-w-lg">
                        @switch ($activity->status)
                        @case ( \App\Enums\StatusTask::SUCCESS )
                        {{ $activity->completed_at->format('d F Y, H:i') }}
                        @break
                        @case ( \App\Enums\StatusTask::LATE )
                        {{ 'Terlambat' }}
                        @break
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center mr-3 max-w-lg">
                        @if ( $activity->status === App\Enums\StatusTask::NOT_STARTED )
                        <!-- Error Badge-->
                        <span
                            class="inline-flex items-center justify-center gap-1 rounded-full bg-error-50 px-2.5 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">
                            {{ $activity->status ?? 'Tidak tersedia' }}
                        </span>
                        @elseif ( $activity->status === App\Enums\StatusTask::LATE )
                        <!-- Error Badge-->
                        <span
                            class="inline-flex items-center justify-center gap-1 rounded-full bg-error-50 px-2.5 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">
                            {{ $activity->status ?? 'Tidak tersedia' }}
                        ...
                        <!-- Warning Badge-->
                        <span
                            class="inline-flex items-center justify-center gap-1 rounded-full bg-warning-50 px-2.5 py-0.5 text-theme-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-orange-400">
                            {{ $activity->status ?? 'Tidak tersedia' }}
                        </span>
                        @elseif ( $activity->status === App\Enums\StatusTask::ON_PROGRESS )
                        <!-- Info Badge-->
                        <span
                            class="inline-flex items-center justify-center gap-1 rounded-full bg-blue-light-50 px-2.5 py-0.5 text-theme-xs font-medium text-blue-light-500 dark:bg-blue-light-500/15 dark:text-blue-light-500">
                            {{ $activity->status ?? 'Tidak tersedia' }}
                        </span>
                        @else
                        <!-- Success Badge-->
                        <span
                            class="inline-flex items-center justify-center gap-1 rounded-full bg-success-50 px-2.5 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">
                            {{ $activity->status ?? 'Tidak tersedia' }}
                        </span>
                        @endif
                    </div>
                        ...
                        <ul class="py-1 text-sm" aria-labelledby="activity-{{ $activity->id }}-dropdown-button">
                            ...
                            {{-- <li>
                                <button data-modal-target="showActivityModal-{{ $activity->id }}"
                                    data-modal-toggle="showActivityModal-{{ $activity->id }}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600
                            dark:hover:text-white text-gray-700 dark:text-gray-200">

                                    <!-- <li><button type="button" data-modal-target="showActivityModal-{{ $activity->id }}"
                                    data-modal-toggle="showActivityModal-{{ $activity->id }}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600
                                    dark:hover:text-white text-gray-700 dark:text-gray-200"> -->
                            ...
                            <li>
                                <button type="button" data-modal-target="deleteactivityModal-{{ $activity->id }}"
                                    data-modal-toggle="deleteactivityModal-{{ $activity->id }}"
                                    class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                    <svg class="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="none"
                                        xmlns="http://www.w3.org/2000/svg" transform="rotate(0 0 0)">
                                        <path
            ...
            <!-- View modal -->
            <div id="showActivityModal-{{ $activity->id }}" tabindex="-1" aria-hidden="true"
                class="hidden fixed top-0 left-0 z-99999 flex h-screen w-full flex-col items-center justify-between overflow-x-hidden bg-white p-6 lg:p-10 dark:bg-gray-900">
                    ...
                    <div class="relative flex h-full w-full flex-col bg-white dark:bg-gray-800">
                        ...
                        <div class="flex items-center justify-between p-4">
                            ...
                            <button type="button"
                                ...
                                data-modal-hide="showActivityModal-{{ $activity->id }}">
            ...
            {{-- <div id="showActivityModal-{{ $activity->id }}" class="fixed top-0 hidden left-0 z-999999 flex h-screen w-full flex-col items-start justify-start overflow-x-hidden
bg-white p-6 lg:p-10 dark:bg-gray-900" style="">
                <!-- close btn -->
                <button data-modal-target="showActivityModal-{{ $activity->id }}"
                    data-modal-toggle="showActivityModal-{{ $activity->id }}"
            ...
            <div id="deleteactivityModal-{{ $activity->id }}"
                ...
                <div data-modal-target="deleteactivityModal-{{ $activity->id }}"
                    data-modal-toggle="deleteactivityModal-{{ $activity->id }}"
                ...
                <div class="relative w-full max-w-[600px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-10">
                    <!-- close btn -->
                    <button data-modal-target="deleteactivityModal-{{ $activity->id }}"
                        data-modal-toggle="deleteactivityModal-{{ $activity->id }}"
                        class="absolute right-3 top-3 z-999 flex h-9.5 w-9.5 items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-colors hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white sm:right-6 sm:top-6 sm:h-11 sm:w-11">
                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                            Apakah Anda yakin ingin menghapus data aktivitas yang satu ini?
                        </p>

                        <div class="flex items-center justify-center w-full gap-3 mt-7">
                            <button data-modal-target="deleteactivityModal-{{ $activity->id }}"
                                data-modal-toggle="deleteactivityModal-{{ $activity->id }}" type="button"
                                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs transition-colors hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200 sm:w-auto">
                                Cancel
                            </button>
                            <form action="{{ route('member.activities.destroy', $activity->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
